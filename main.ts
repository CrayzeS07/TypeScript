...
if (userDetails) {
  console.log(
    `User ID ${userId}'s name is ${userDetails.name} and address is ${userDetails.address}`
  );
} else {
...
//masala 3
function getMaxNumbers(arr1: number[], arr2: number[]): string[] {
  const maxNumbers: string[] = [];
